...
using JohnStriker.GameObjects.FogSphereRenderer;
using JohnStriker.GameObjects.VolumetricLightRenderer;
using JohnStriker.GameObjects.Water;
...
{
    internal sealed class MyGraphicsScreen : DigitalRune.Graphics.GraphicsScreen, IDisposable
    {
        #region Fields
        ...
        public MyGraphicsScreen(IServiceLocator services)
            : base(services.GetInstance<IGraphicsService>())
        {
            ...
            _planarReflectionRenderer = new PlanarReflectionRenderer(context =>
            {
                ...
                {
                    int index = sceneQuery.RenderableNodes.IndexOf(planarReflectionNode.Parent);
                    if (index >= 0)
        ...
        protected override void OnRender(RenderContext context)
        {
            ...
            foreach (SceneNode t in query.SceneNodes)
            {
                //skyfighter fbx
                if (t.Parent.Name == "skyfighter fbx")
                {
                    ...
                    MeshRenderer.Render(t, context);
                    context.RenderPass = null;
                }

                if (t.Parent.Name == "RustyCube")
                {
                    // Set the render states for opaque objects.
                    graphicsDevice.DepthStencilState = DepthStencilState.Default;
                    graphicsDevice.RasterizerState = RasterizerState.CullCounterClockwise;
        ...
        private void RenderScene(CustomSceneQuery sceneQuery, RenderContext context,
            bool doPostProcessing, bool renderLensFlares, bool renderDebugOutput, bool renderReticle)
        {
            RenderTargetPool renderTargetPool = GraphicsService.RenderTargetPool;
            GraphicsDevice graphicsDevice = GraphicsService.GraphicsDevice;
            RenderTarget2D originalRenderTarget = context.RenderTarget;
            Viewport originalViewport = context.Viewport;
            Texture2D originalSourceTexture = context.SourceTexture;
            ...
            else
            {
                ...
                SpriteBatch.Draw(context.SourceTexture,
                    new Rectangle(0, 0, originalViewport.Width, originalViewport.Height), Color.White);
                SpriteBatch.End();
            }
            ...
            if (renderDebugOutput)
            {
                ...
                if (VisualizeIntermediateRenderTargets)
                {
                    ...
                    {
                        RenderTarget2D shadowMask = ShadowMaskRenderer.ShadowMasks[i];
                        if (shadowMask != null)
            ...
            if (renderReticle)
            {
                ...
                SpriteBatch.Draw(
                    _reticle,
                    new Vector2(originalViewport.Width/2 - _reticle.Width/2,
                        originalViewport.Height/2 - _reticle.Height/2),
                    Color.Black);
                SpriteBatch.End();
            }
        ...
        {
            RenderTargetPool renderTargetPool = GraphicsService.RenderTargetPool;
            foreach (SceneNode node in lights)
            {
        ...
        private static bool IsCameraUnderwater(CustomSceneQuery query, CameraNode cameraNode)
        {
            ...
            foreach (SceneNode node in query.RenderableNodes)
            {
