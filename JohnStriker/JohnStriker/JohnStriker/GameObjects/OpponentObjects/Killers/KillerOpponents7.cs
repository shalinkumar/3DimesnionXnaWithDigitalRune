...
{
    public class KillerOpponents7 : GameObject
    {
        #region Constants
        ...
        public KillerOpponents7(IServiceLocator services, int objectCount)
        {
    ...
    (PlayerObjects.Player)GameObjectService.Objects["VehicleOpponent"];
                ...
                if ((_models[i].PoseWorld.Position - playerNode.PoseWorld.Position).Length <= 500)
                {
                    ...
                    if (killerState == KillerAiState.Chasing)
                    {
                        ...
                        if (cameraGameObjects._fRoll > 0.0f)
                        {
                            _fRoll += 0.04f;
                        }
                        ...
                        //_shipRotation.Y = 0.0f;
                        //_shipRotation.X = 0.0f;
                        //_shipRotation.Z = -49.5f;
                        modelOrientation = QuaternionF.CreateRotationZ(MathHelper.ToRadians(_shipRotation.Z));

                        //_shipRotation = new Vector3F(0, 0, forwardModelPose.Z);

                        //forwardModelPose = modelOrientation.Rotate(_shipRotation);

                        Vector3F translation = forwardModelPose * _linearVelocityMagnitude * deltaTimeF;

                        var pose = new Pose(modelOrientation);

                        //_bodies[i].Pose = new Pose(_models[i].PoseWorld.Position + translation, newPose.Orientation + pose.Orientation);

                        _bodies[i].Pose = new Pose(_models[i].PoseWorld.Position + translation, (new Matrix33F(newPose.Orientation.M00, newPose.Orientation.M01, newPose.Orientation.M02, newPose.Orientation.M10, newPose.Orientation.M11, newPose.Orientation.M12, newPose.Orientation.M20, newPose.Orientation.M21, newPose.Orientation.M22)));
