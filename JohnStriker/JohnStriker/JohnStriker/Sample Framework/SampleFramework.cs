...
namespace JohnStriker
{
    ...
    public class SampleFramework
    {
        ...
        public SampleFramework(Game1 game)
        {
            _game = game;
...
#if NETFX_CORE || WINDOWS_PHONE || ANDROID || IOS || LINUX || MACOS
      _nextSampleIndex = _samples.IndexOf(typeof(Graphics.BasicEffectSample));
#elif KINECT
      _nextSampleIndex = _samples.IndexOf(typeof(Kinect.KinectSkeletonMappingSample));
#else
            _nextSampleIndex = _samples.IndexOf(typeof(GameMenu));
#endif
        }
      ...
      button.Padding = new Vector4F(15);
        ...
        private void LoadSample(int sampleIndex)
        {
            ...
            _sample = (GameManager)_services.CreateInstance(type);
      ...
      foreach (var hierarchicalProfiler in hierarchicalProfilers)
      {
        ...
        public Panel AddOptions(string tabName, int tabIndex)
        {
            ...
            var optionsPanel = _optionsTabControl.Items
                                                 ...
                                                 .Select(tabItem => (Panel)tabItem.TabPage)
                                                 .FirstOrDefault();
